...
public class WebGLContext : RenderingContext
{
    ...
    public void TexImage2D<T>(Texture2DType target, int level, PixelFormat internalFormat, int width, int height, int border, PixelFormat format, PixelType type, T[] pixels)
        where T : struct
        => CallMethod<object>(TEX_IMAGE_2D, target, level, internalFormat, width, height, border, format, type, pixels);
    public async Task TexImage2DAsync<T>(Texture2DType target, int level, PixelFormat internalFormat, int width, int height, int border, PixelFormat format, PixelType type, T[] pixels)
        where T : struct
        => await BatchCallAsync(TEX_IMAGE_2D, isMethodCall: true, target, level, internalFormat, width, height, border, format, type, pixels);

       
    public void TexImage2D(Texture2DType target, int level, PixelFormat internalFormat, PixelFormat format, PixelType type, ElementReference img)
        => CallMethod<object>(TEX_IMAGE_2D, target, level, internalFormat, format, type, img);
    public async Task TexImage2DAsync(Texture2DType target, int level, PixelFormat internalFormat, PixelFormat format, PixelType type, ElementReference img)
        => await BatchCallAsync(TEX_IMAGE_2D, isMethodCall: true, target, level, internalFormat, format, type, img);
    ...
    // we use jsIn (marshalls the data) for registering calls (1x) 
    // we use jsUn for using the data via id

    public void PinGLEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "pinGLEx");
        jsIn.Invoke<object>("BlazorExtensions.WebGL.pinGLEx", Canvas);
    }

    public WebGLPin CreatePinEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "createPinEx");
        return jsIn.Invoke<WebGLPin>("BlazorExtensions.WebGL.createPinEx", Canvas);
    }

    public void UsePinEx(WebGLPin pin)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "usePinEx");
        jsUn.InvokeUnmarshalled<int, object>("BlazorExtensions.WebGL.usePinEx", pin.Id);
    }

    // Register fixed/static transformations as JavaScript objects:
    public WebGLUniformMatrix CreateUniformMatrixEx(float[] floats)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "createUniformMatrixEx");
        return jsIn.Invoke<WebGLUniformMatrix>("BlazorExtensions.WebGL.createUniformMatrixEx", floats);
    }

    public void RenderUniformMatrixEx(WebGLUniformLocation location, WebGLUniformMatrix value)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "renderUniformMatrixEx");
        jsUn.InvokeUnmarshalled<int, int, object>("BlazorExtensions.WebGL.renderUniformMatrixEx", location.Id, value.Id);
    }

    // according to docs but slightly slower than RenderUniformMatrixEx?
    public void RenderUniformMatrixIEx(WebGLUniformLocation location, WebGLUniformMatrix value)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "renderUniformMatrixIEx");
        var interop = new InteropUniformMatrix4fv { LocationId = location.Id, MatrixId = value.Id };
        jsUn.InvokeUnmarshalled<InteropUniformMatrix4fv, object>("BlazorExtensions.WebGL.renderUniformMatrixIEx", interop);
    }

    public void UniformMatrix4fvEx(WebGLUniformLocation uniform, float[] floats)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "uniformMatrix4fvEx");
        jsUn.InvokeUnmarshalled<int, float[], object>("BlazorExtensions.WebGL.uniformMatrix4fvEx", uniform.Id, floats);
    }

    public void DrawElementsEx(int count)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "drawElementsEx");
        jsUn.InvokeUnmarshalled<int, object>("BlazorExtensions.WebGL.drawElementsEx", count);
    }

    public void BindFloatBufferEx(uint attribute, WebGLBuffer buffer, int valueSize)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "bindFloatBufferEx");
        jsUn.InvokeUnmarshalled<uint, int, int, object>("BlazorExtensions.WebGL.bindFloatBufferEx", attribute, buffer.Id, valueSize);
    }

    public void BindUShortBufferEx(WebGLBuffer buffer)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "bindUShortBufferEx");
        jsUn.InvokeUnmarshalled<int, object>("BlazorExtensions.WebGL.bindUShortBufferEx", buffer.Id);
    }

    public void BindTextureEx(WebGLUniformLocation uniform, WebGLTexture texture)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "bindTextureEx");
        jsUn.InvokeUnmarshalled<int, int, object>("BlazorExtensions.WebGL.bindTextureEx", uniform.Id, texture.Id);
    }

    public void BlendFuncEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "blendFuncEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.blendFuncEx");
    }

    public void EnableBlendEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "enableBlendEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.enableBlendEx");
    }

    public void DisableBlendEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "disableBlendEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.disableBlendEx");
    }

    public void ClearDepthEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "clearDepthEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.clearDepthEx");
    }

    public void ClearDepthAndColorEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "clearDepthAndColorEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.clearDepthAndColorEx");
    }

    public void UseProgramEx(WebGLProgram program)
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "useProgramEx");
        jsUn.InvokeUnmarshalled<int, object>("BlazorExtensions.WebGL.useProgramEx", program.Id);
    }

    public void DepthMaskOffEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "depthMaskOffEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.depthMaskOffEx");
    }

    public void DepthMaskOnEx()
    {
        using PerfMeasure perfMeasure = new PerfMeasure(Context, "depthMaskOnEx");
        jsUn.InvokeUnmarshalled<object>("BlazorExtensions.WebGL.depthMaskOnEx");
    }
